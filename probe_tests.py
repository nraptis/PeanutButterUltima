...
# Probe matrix knobs.
TEST_LOOPS = [10]
BLOCK_DATA_SIZES = [1, 2, 3]
TEST_BLOCK_COUNTS = [1, 2, 3]
...
def write_test_config(
    ...
    text = _replace_tagged_define_line(text, TAG_L3, block_data_size)
    text = _replace_tagged_define_line(text, TAG_SEED, test_seed)
    text = _replace_tagged_define_line(text, TAG_LOOPS, test_loops)
    text = _replace_tagged_define_line(text, TAG_BLK_CNT, test_block_count)
...
def run_probe() -> None:
    ...
    run_matrix = list(itertools.product(
        BLOCK_DATA_SIZES,
        TEST_SEEDS,
        TEST_LOOPS,
        TEST_BLOCK_COUNTS,
    ...
    try:
        for run_index, (block_data_size, test_seed, test_loops, test_block_count) in enumerate(run_matrix, start=1):
            print(f"Starting Run {run_index} of {total_runs}")
            if block_data_size <= 0 or test_block_count < 1:
                for test_file in test_files:
                    fail_count += 1
                    report_lines.append(
                        f"[Fail] {test_file.name} L3={block_data_size} SEED={test_seed} LOOPS={test_loops} BLOCKS={test_block_count} :: invalid test constants (BLOCK_SIZE_L3 > 0 and TEST_BLOCK_COUNT >= 1 required)"
                    )
                continue
            ...
            if build_rc != 0:
                ...
                for test_file in test_files:
                    fail_count += 1
                    report_lines.append(
                        f"[Fail] {test_file.name} L3={block_data_size} SEED={test_seed} LOOPS={test_loops} BLOCKS={test_block_count} :: build failed: {summary}"
                    )
                continue
            ...
            for test_file in test_files:
                ...
                if rc == 0:
                    pass_count += 1
                    report_lines.append(
                        f"[Pass] {test_file.name} L3={block_data_size} SEED={test_seed} LOOPS={test_loops} BLOCKS={test_block_count}"
                    )
                else:
                    fail_count += 1
                    summary = extract_failure_summary(output)
                    report_lines.append(
                        f"[Fail] {test_file.name} L3={block_data_size} SEED={test_seed} LOOPS={test_loops} BLOCKS={test_block_count} :: {summary}"
                    )
    finally:
