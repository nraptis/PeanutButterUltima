...
int main() {
  ...
  const ArchiveMutator aMutator = [](peanutbutter::ultima::ByteVector& pBytes, std::string* pErrorMessage) {
    ...
    pBytes[aOffset + 0] = 100;
    pBytes[aOffset + 1] = 0x00;
    return true;
  };

  std::string aError;
  if (!RunUnbundleIntegerFailureCase("UNP_FNL_002", SeedBasicIntegerFailureInputTree, aMutator, &aError)) {
    std::cerr << aError << "\n";
    return 1;
  }
  std::cout << "Test_IntegerFailureCases_UNP_FNL_002 passed\n";
  return 0;
}
