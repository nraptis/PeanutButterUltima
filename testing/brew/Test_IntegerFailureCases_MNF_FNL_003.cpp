...
int main() {
  ...
  const ArchiveMutator aMutator = [](peanutbutter::ByteVector& pBytes, std::string* pErrorMessage) {
    ...
    pBytes[aLogicalToPhysical[aManifestLengthOffset + 0]] = 0xFF;
    pBytes[aLogicalToPhysical[aManifestLengthOffset + 1]] = 0xFF;
    return true;
  };

  std::string aError;
  if (!RunUnbundleIntegerFailureCase("MNF_FNL_003", SeedManifestIntegerFailureInputTree, aMutator, &aError)) {
    std::cerr << aError << "\n";
    return 1;
  }
  std::cout << "Test_IntegerFailureCases_MNF_FNL_003 passed\n";
  return 0;
}
