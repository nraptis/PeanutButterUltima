...
int main() {
  ...
  const ArchiveMutator aMutator = [](peanutbutter::ByteVector& pBytes, std::string* pErrorMessage) {
    ...
    if (aManifestLengthOffset + 2 >= aLogicalToPhysical.size()) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "Mutation failed: manifest payload too small for MNF_FNL_002 mutation.";
      }
      return false;
    }
    pBytes[aLogicalToPhysical[aManifestLengthOffset + 0]] = 0x10;
    pBytes[aLogicalToPhysical[aManifestLengthOffset + 1]] = 0x00;
    const std::size_t aTruncateAt = aLogicalToPhysical[aManifestLengthOffset + 2] + 1;
    if (aTruncateAt >= pBytes.size()) {
      if (pErrorMessage != nullptr) {
        *pErrorMessage = "Mutation failed: could not truncate archive for MNF_FNL_002.";
      }
      return false;
    }
    pBytes.resize(aTruncateAt);
    return true;
  };

  std::string aError;
  if (!RunUnbundleIntegerFailureCase("MNF_FNL_002", SeedManifestIntegerFailureInputTree, aMutator, &aError)) {
    std::cerr << aError << "\n";
    return 1;
  }
  std::cout << "Test_IntegerFailureCases_MNF_FNL_002 passed\n";
  return 0;
}
