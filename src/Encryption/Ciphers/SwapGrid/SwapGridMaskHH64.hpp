#ifndef JELLY_SWAP_GRID_MASK_HH64_HPP_
#define JELLY_SWAP_GRID_MASK_HH64_HPP_
...
class SwapGridMaskHH64 final : public Crypt {
 public:
  explicit SwapGridMaskHH64(std::uint8_t pMask) : mMask(pMask) {}
  ...
  bool Apply(const unsigned char* pSource,
    ...
    for (std::size_t aOffset = 0; aOffset < pLength; aOffset += kTileSize) {
      for (std::size_t aRow = 0; aRow < kDim; ++aRow) {
        ...
        const std::size_t aSourceBase = aDestinationBase;
        pDestination[aDestinationBase + 0] =
            Blend(pSource[aDestinationBase + 0], pSource[aSourceBase + 1], mMask, aAntiMask);
        pDestination[aDestinationBase + 1] =
            Blend(pSource[aDestinationBase + 1], pSource[aSourceBase + 0], mMask, aAntiMask);
        pDestination[aDestinationBase + 2] =
            Blend(pSource[aDestinationBase + 2], pSource[aSourceBase + 3], mMask, aAntiMask);
        pDestination[aDestinationBase + 3] =
            Blend(pSource[aDestinationBase + 3], pSource[aSourceBase + 2], mMask, aAntiMask);
        pDestination[aDestinationBase + 4] =
            Blend(pSource[aDestinationBase + 4], pSource[aSourceBase + 5], mMask, aAntiMask);
        pDestination[aDestinationBase + 5] =
            Blend(pSource[aDestinationBase + 5], pSource[aSourceBase + 4], mMask, aAntiMask);
        pDestination[aDestinationBase + 6] =
            Blend(pSource[aDestinationBase + 6], pSource[aSourceBase + 7], mMask, aAntiMask);
        pDestination[aDestinationBase + 7] =
            Blend(pSource[aDestinationBase + 7], pSource[aSourceBase + 6], mMask, aAntiMask);
      }
    }
    return true;
  }
...
#endif  // JELLY_SWAP_GRID_MASK_HH64_HPP_
