#ifndef JELLY_SPLINT_MASK_BYTE_BLOCK_CIPHER_HPP_
#define JELLY_SPLINT_MASK_BYTE_BLOCK_CIPHER_HPP_
...
class SplintMaskByteBlockCipher final : public Crypt {
 public:
  SplintMaskByteBlockCipher(std::size_t pBlockSize, std::uint8_t pMask)
 ...
 private:
  ...
  bool ApplyEncryptSoftware(const unsigned char* pSource,
    ...
    SplintEncryptBlocks(aWorker, pDestination, pLength);
  ...
  bool ApplyDecryptSoftware(const unsigned char* pSource,
    ...
    unsigned char* aWorker = const_cast<unsigned char*>(pWorker);
    MaskCopyScalar(pSource, aWorker, pLength);
    SplintDecryptBlocks(aWorker, pDestination, pLength);
    AntiMaskOrScalar(pSource, pDestination, pLength);
    return true;
  }
  ...
  void SplintEncryptBlocks(const unsigned char* pSource,
                           unsigned char* pDestination,
                           std::size_t pLength) const {
    ...
    for (std::size_t aIndex = 0; aIndex < aBlockCount; ++aIndex) {
      SplintEncryptSpan(aBlock, aOut, mBlockSize);
      aBlock += mBlockSize;
      aOut += mBlockSize;
    }
  }

  void SplintDecryptBlocks(const unsigned char* pSource,
                           unsigned char* pDestination,
                           std::size_t pLength) const {
    const std::size_t aBlockCount = pLength / mBlockSize;
    const unsigned char* aBlock = pSource;
    ...
    for (std::size_t aIndex = 0; aIndex < aBlockCount; ++aIndex) {
      SplintDecryptSpan(aBlock, aOut, mBlockSize);
      aBlock += mBlockSize;
      aOut += mBlockSize;
    }
  }

  static void SplintEncryptSpan(const unsigned char* pSource,
                                unsigned char* pDestination,
                                std::size_t pLength) {
  ...
  static void SplintDecryptSpan(const unsigned char* pSource,
                                unsigned char* pDestination,
                                std::size_t pLength) {
...
#endif  // JELLY_SPLINT_MASK_BYTE_BLOCK_CIPHER_HPP_
